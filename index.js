  ...
  try {
    ...
    for (const model of lote) {
      ...
      if (docOffline.exists) {
        ...
        console.log(`[VOLTOU] A modelo ${model.username} voltou! Restaurando post...`);
        ...
        try {
          ...
          // Aqui está o seu link I'Love Prive
        ...
        if (descPesquisa.length > 150) { descPesquisa = descPesquisa.substring(0, 147) + '...'; }
        ...
        // Aqui também está o link I'Love Prive configurado
        const linkDireto = `https://iloveprive.com/${model.username}`;
        ...
        const tagsBlogger = Array.from(
          new Set(labelsRaw.filter(label => label && label.trim() !== '').map(label => label.trim()))
        ).filter((label, _, arr) => {
